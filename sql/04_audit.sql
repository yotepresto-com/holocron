...
BEGIN
  IF NOT EXISTS (
    ...
    FROM
      pg_type
    WHERE
      typname = 'operation_type_enum') THEN
...
BEGIN
  ...
  END IF;
  -- Insert audit log
  INSERT INTO audit_log (table_name, operation_type, record_id, changed_data, changed_by)
...
BEGIN
  ...
  -- Insert audit log
  INSERT INTO audit_log (table_name, operation_type, record_id, changed_data, changed_by)
    VALUES (TG_TABLE_NAME, 'UPDATE', NEW.id, to_jsonb (NEW), current_setting('app.current_user_id')::INTEGER);
  RETURN NEW;
END;
$$
LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION set_current_user_id (_user_id INTEGER)
  RETURNS VOID
  AS $$
BEGIN
  PERFORM
    set_config('app.current_user_id', _user_id::TEXT, TRUE);
END;
$$
LANGUAGE plpgsql;
