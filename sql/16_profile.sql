...
CREATE TABLE IF NOT EXISTS profile_attribute (
  ...
  description TEXT,
  type VARCHAR(50) NOT NULL CHECK (type IN ('numerical', 'date', 'categorical')),
  is_transactional BOOLEAN NOT NULL DEFAULT FALSE
);

CREATE INDEX IF NOT EXISTS idx_profile_attribute_profile_id ON profile_attribute (profile_id);
...
CREATE INDEX IF NOT EXISTS idx_profile_person_attribute_id ON profile_data (person_id, attribute_id);
